...
async function main() {
    ...
    const vehicles = await Promise.all(Array(20)
        .fill(null)
        .map((_, i) => {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        const randomTechnician = activeTechnicians[Math.floor(Math.random() * activeTechnicians.length)];
    ...
    for (const type of commentTypes) {
        await Promise.all(commentTemplates[type].map((comment, index) => prisma.comments.create({
            data: {
                type,
                comment,
                created_at: new Date(),
